...
public class PlantController {
    ...
    public ResponseEntity<ResponceDto> getAllPlants() {
        try {
            ...
            return new ResponseEntity<>(new ResponceDto("Success", plants, false), HttpStatus.CREATED);
        } catch (RuntimeException e) {
            return new ResponseEntity<>(new ResponceDto("Failed", e.getMessage(), true), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    @GetMapping("/{id}")
    public ResponseEntity<ResponceDto> getPlantById(@PathVariable UUID id) {
        try {
            Plant plant = plantService.getPlantById(id);
            return new ResponseEntity<>(new ResponceDto("Success", plant, false), HttpStatus.CREATED);
        } catch (RuntimeException e) {
            return new ResponseEntity<>(new ResponceDto("Failed", e.getMessage(), true), HttpStatus.NOT_FOUND);
        }
    }
    ...
    public ResponseEntity<ResponceDto> createPlant(@RequestBody Plant plant) {
        try {
            ...
            return new ResponseEntity<>(new ResponceDto("Failed", e.getMessage(), true), HttpStatus.BAD_REQUEST);
        }
    }

    @PutMapping("/{id}")
    public ResponseEntity<ResponceDto> updatePlant(@PathVariable UUID id, @RequestBody Plant plant) {
        try {
            Plant updatedPlant = plantService.updatePlant(id, plant);
            return new ResponseEntity<>(new ResponceDto("Success", updatedPlant, false), HttpStatus.CREATED);
        } catch (RuntimeException e) {
            return new ResponseEntity<>(new ResponceDto("Failed", e.getMessage(), true), HttpStatus.NOT_FOUND);
        }
    }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<ResponceDto> deletePlant(@PathVariable UUID id) {
        try {
            plantService.deletePlant(id);
            return new ResponseEntity<>(new ResponceDto("Success", "Deleted", false), HttpStatus.CREATED);
        } catch (RuntimeException e) {
            return new ResponseEntity<>(new ResponceDto("Failed", e.getMessage(), true), HttpStatus.NOT_FOUND);
        }
    }
